...
# Разрешаем нашему HTML-интерфейсу слать запросы с любого хостинга (NetlifyGitHub)
...
@app.get(apicars, response_model=List[CarItem])
def get_cars()
    with get_db_connection() as conn
...
def save_cars(cars List[CarItem])
    with get_db_connection() as conn
        with conn.cursor() as cur
            cur.execute(DELETE FROM pwa_cars) # Перезаписываем конфигурацию гаража
            for car in cars
                cur.execute(
...
@app.get(apirefuels, response_model=List[RefuelItem])
def get_refuels()
    with get_db_connection() as conn
        with conn.cursor(cursor_factory=RealDictCursor) as cur
...
def save_refuels(refuels List[RefuelItem])
    with get_db_connection() as conn
        with conn.cursor() as cur
            cur.execute(DELETE FROM pwa_refuels) # Синхронизируем массив заправок
            for r in refuels
                cur.execute(
